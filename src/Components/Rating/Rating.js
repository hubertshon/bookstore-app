...
export const Rating = (props) => {
    ...
    return <div className="rating-container">
        {[1, 2, 3, 4, 5].map((number) => 
            number > props.stars ? <FontAwesomeIcon key={number} icon={faStar} size="sm" color="#DDDDDD" className="star" onClick={(e) => handleStarClick(number, e)} /> 
            :
            <FontAwesomeIcon key={number} icon={faStar} size="sm" color="#5171A5" className="star" onClick={(e) => handleStarClick(number, e)} />
        )}
